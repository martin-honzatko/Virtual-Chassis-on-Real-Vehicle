...
public class ConfigManager : MonoBehaviour
{
    ...
    void Start()
    {
        try
        {
            ...
            ConfigCatalog configCatalog = JSONReader.LoadConfigCatalog(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "chassisSend.json")); //Loading configuration file.

            if (configCatalog == null) //Controling the whether the file exists or not.
            {
                //MainController.Log("Configuration not available");

                myDisplay.GetComponent<TextMeshPro>().text = "Configuration not available! The chassisSend.json file is not in the picture directory or the file is damaged.";
            }

            else
            {
                SetConfiguration(configCatalog); //Setting configs from the file.
                //myDisplay.GetComponent<TextMeshPro>().text = $"IP: {externalIP}, Port: {port}";
        ...
        catch (Exception ex)
        {
            ...
            Debug.Log($"Network configuration ERROR: {ex.Message}");
        }
